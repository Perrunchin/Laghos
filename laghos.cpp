...
int main(int argc, char *argv[])
{
    ...
    const int partitions = floor(pow(num_tasks, 1.0 / dim) + 1.0e-2);
    ...
    else
    {
        if (myid == 0)
        {
            ...
            cout << "MFEM was built without METIS. Adjust the number of tasks to use a Cartesian split." << endl;
#endif
        }
#ifndef MFEM_USE_METIS
        return 1;
    ...
    if (mpi.Root())
    {
        cout << "Number of kinematic (position, velocity) dofs: " << glob_size_h1 << endl;
        cout << "Number of specific internal energy dofs: " << glob_size_l2 << endl;
    }
    ...
    if (visualization)
    {
        ...
        VisualizeField(vis_v, vishost, visport, v_gf, "Velocity", Wx, Wy, Ww, Wh);
        Wx += offx;
    ...
    for (int ti = 1; !last_step; ti++)
    {
        ...
        if (last_step || (ti % vis_steps) == 0)
        {
            ...
            if (gfprint)
            {
                ...
                mesh_name << basename << "_" << ti << "_mesh." << setfill('0') << setw(6) << myid;
                rho_name  << basename << "_" << ti << "_rho." << setfill('0') << setw(6) << myid;
                v_name << basename << "_" << ti << "_v." << setfill('0') << setw(6) << myid;
                e_name << basename << "_" << ti << "_e." << setfill('0') << setw(6) << myid;
        ...
        case 0:
            {
                ...
                if (x.Size() == 2)
                {
                    val = 1.0 + (cos(2.0 * M_PI * x(0)) + cos(2.0 * M_PI * x(1))) / 4.0;
                }
                else
                {
                    val = 100.0 + ((cos(2.0 * M_PI * x(2)) + 2.0) * (cos(2.0 * M_PI * x(0)) + cos(2.0 * M_PI * x(1))) - 2.0) / 16.0;
                }
