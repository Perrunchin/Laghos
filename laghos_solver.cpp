...
void VisualizeField(socketstream &sock, const char *vishost, int visport, ParGridFunction &gf, const char *title, int x, int y, int w, int h, bool vec)
{
...
LagrangianHydroOperator::LagrangianHydroOperator(int size,
    ...
    integ_rule(IntRules.Get(h1_fes.GetMesh()->GetElementBaseGeometry(), 3 * h1_fes.GetOrder(0) + l2_fes.GetOrder(0) - 1)),
    ...
    locCG(), timer()
{
    ...
    {
        mi.AssembleElementMatrix(*l2_fes.GetFE(i), *l2_fes.GetElementTransformation(i), Me);
        inv.Factor();
    ...
    for (int i = 0; i < nzones; i++)
    {
        ...
        for (int q = 0; q < nqp; q++)
        {
            ...
            quad_data.rho0DetJ0w(i*nqp + q) = rho0DetJ0 * integ_rule.IntPoint(q).weight;
        }
    }
    ...
    if (p_assembly)
    {
        tensors1D = new Tensors1D(H1FESpace.GetFE(0)->GetOrder(), L2FESpace.GetFE(0)->GetOrder(), int(floor(0.7 + pow(nqp, 1.0 / dim))));
...
void LagrangianHydroOperator::Mult(const Vector &S, Vector &dS_dt) const
{
    ...
    if (p_assembly)
    {
        ...
        for (int c = 0; c < dim; c++)
        {
            Vector rhs_c(rhs.GetData() + c * size, size), dv_c(dv.GetData() + c * size, size);
            Array<int> c_tdofs;
...
double LagrangianHydroOperator::GetTimeStepEstimate(const Vector &S) const
{
    ...
    double glob_dt_est;
    MPI_Allreduce(&quad_data.dt_est, &glob_dt_est, 1, MPI_DOUBLE, MPI_MIN, H1FESpace.GetParMesh()->GetComm());
...
void LagrangianHydroOperator::ComputeDensity(ParGridFunction &rho)
{
    ...
    for (int i = 0; i < nzones; i++)
    {
        di.AssembleRHSElementVect(*L2FESpace.GetFE(i), *L2FESpace.GetElementTransformation(i), rhs);
        mi.AssembleElementMatrix(*L2FESpace.GetFE(i), *L2FESpace.GetElementTransformation(i), Mrho);
        inv.Factor();
        inv.Mult(rhs, rho_z);
...
void LagrangianHydroOperator::PrintTimingData(bool IamRoot, int steps)
{
    ...
    if (IamRoot)
    {
        ...
        cout << "CG (H1) rate (megadofs x cg_iterations / second): " << 1e-6 * timer.H1dof_iter / rt_max[0] << endl;
        cout << endl;
        cout << "CG (L2) total time: " << rt_max[1] << endl;
        cout << "CG (L2) rate (megadofs x cg_iterations / second): " << 1e-6 * alldata[0] / rt_max[1] << endl;
        cout << endl;
        cout << "Forces total time: " << rt_max[2] << endl;
        cout << "Forces rate (megadofs x timesteps / second): " << 1e-6 * timer.dof_tstep / rt_max[2] << endl;
        cout << endl;
        cout << "UpdateQuadData total time: " << rt_max[3] << endl;
        cout << "UpdateQuadData rate (megaquads x timesteps / second): " << 1e-6 * alldata[1] / rt_max[3] << endl;
        cout << endl;
        cout << "Major kernels total time (seconds): " << rt_max[4] << endl;
        cout << "Major kernels total rate (megadofs x time steps / second): " << 1e-6 * H1FESpace.GlobalTrueVSize() * steps / rt_max[4] << endl;
    }
}
...
void LagrangianHydroOperator::UpdateQuadratureData(const Vector &S) const
{
    ...
    DenseMatrix Jpi(dim), sgrad_v(dim), Jinv(dim), stress(dim), stressJiT(dim), vecvalMat(vector_vals.GetData(), h1dofs_cnt, dim);
    ...
    double *e_b = new double[nqp_batch];
    double *p_b = new double[nqp_batch];
    double *cs_b = new double[nqp_batch];
    ...
    for (int b = 0; b < nbatches; b++)
    {
        ...
        for (int z = 0; z < nzones_batch; z++)
        {
            ...
            for (int q = 0; q < nqp; q++)
            {
                ...
                const double h_min = Jpr.CalcSingularvalue(dim - 1) / (double) H1FESpace.GetOrder(0);
